# -*- coding: utf-8 -*-
"""
Created on Mon May  5 16:18:35 2025
...
def cross_val_goal_metrics(X, y, model, goal="goal1"):
    ...
    for train_idx, val_idx in skf.split(X, y):
        ...
        elif goal == 'goal3':  # General performance (maximize F1)
...
def scorer_goal3(estimator, X, y):
    y_prob = estimator.predict_proba(X)[:, 1]
    thresholds = np.linspace(0.01, 0.99, 99)
...
def train_and_evaluate_model(model_name, model, param_grid, preprocessor, X_train, y_train, X_test, y_test):
    """
    Train and evaluate a model for all 3 business goals (goal1, goal2, goal3).
...
def plot_pr_curves(results, X_test, y_test):
    ...
    goals = {
        ...
        'goal3': {'title': 'Goal 3 (Maximize F1)'}
    }
...
def visualize_decision_tree(model, goal_name, feature_names):
    ...
    # Plot the tree
    plt.figure(figsize=(60, 80))  # Big size for better readability
    plot_tree(
        classifier,
        filled=True,
...
# === Visualize the DecisionTree model trained for Goal 3 ===
dtree = results['DecisionTree']['goal3']['model']

# Extract feature names from the preprocessor
preprocessor.fit(X_train)
feature_names = preprocessor.get_feature_names_out()

visualize_decision_tree(dtree, "Goal 3", feature_names)
...
def generate_shap_beeswarm(results, X_train, X_test):
    ...
    for goal in ['goal1', 'goal2', 'goal3']:
        ...
        if goal == 'goal1':
            ...
            best_model = results['DecisionTree']['goal3']['model']
...
# === Get confusion matrix for validation and test set ===
